...
<div class="display-list-book">
  ...
  <form class="book-trip-list2" method="POST">
    ...
    <label>Enter your Email ID:<input type="text" name="email"></label>
    <input type="submit" class="btn btn-primary btn-lg" name="submit" value="View">
    <input type="reset" class="btn btn-secondary btn-lg" name="reset" value="Reset">
  </form>
  <div>
    <a href="delete.php"><button type="button" class="btn btn-primary btn-lg">Delete Trip</button></a>
  <a href="update.php"><button type="button" class="btn btn-secondary btn-lg">Update Trip</button></a>
  </div>
  
</div>

<div class="list-display-area">
<?php
...
$sql = "SELECT `name`, `email`, `Address`, `aadhar`, `phone`, `tripid` FROM `book-trip` WHERE `email`='$u'";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        ." - Address: " . $row["Adress"]. "<br>"."- Aadhar: " . $row["aadhar"]. "<br>"." - Phone Number: " . $row["phone"]. "<br>"
        ." - Trip ID: " . $row["tripid"];
    }
} else {
    echo "0 results";
